...
$o_Listado = $a_mod;
//echo date(Config_L::p('f_fecha_corta'), strtotime($_SESSION['filtro']['fechaDw'])) . ' - ' .date(Config_L::p('f_fecha_corta'), strtotime($_SESSION['filtro']['fechaHw'])).'<br />';
if (isset($o_Listado) && !is_null($o_Listado)) {
    echo '<table class="tb_100 table-striped table-hover">';
    echo '<thead>';
    echo '<tr>';
    echo '<th class="tb_80">';
    echo _('Persona');
...
echo '<div id="CantLlegadasTarde" style="display:none;">' . $cantidad . '</div>';
